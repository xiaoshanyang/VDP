$(function () {
    ...
    $('#rootwizard-custom-circle').bootstrapWizard({
        onTabShow: function(tab, navigation, index) {
            ...
            $('#btnTest').addClass("hidden");
            $('#btnLast').addClass("hidden");

            $('.note').addClass("hidden");

            if ($current == 1 || $current ==2) {
                $('#btnTest').removeClass("hidden");
            } else {
                $('#btnTest').addClass("hidden");
            }
            if ($current == $total) {
        ...
        'onNext': function(tab, navigation, index) {
            ...
            $($id + " input").each(function(){
                if (!$(this).val()) {
                    ...
                    if (index == 4 && ($('#txtReAdminpwd').val() != $('#txtAdminpwd').val())) {
                        $('#txtReAdminpwd').css('border-color', 'red');
                        $('#txtReAdminpwd').parent().parent().find("i.alert").removeClass("alert-hide");
                        $approved = 0;
                    } else {
        ...
        'tabClass': 'bwizard-steps-o','nextSelector': '.button-next', 'previousSelector': '.button-previous', 'lastSelector': '.button-last'
    });
});